...
class CrossrefNlmCitationSchemaFilter extends NlmCitationSchemaFilter {
	...
	function &process(&$citationDescription) {
		...
		if (is_null($resultXml =& $this->callWebService(CROSSREF_WEBSERVICE_URL, $searchParams, XSL_TRANSFORMER_DOCTYPE_STRING)) || StringUtils::substr(trim($resultXml), 0, 6) == '<html>') return $nullVar;

		// Remove default name spaces from XML as CrossRef doesn't
		// set them reliably and element names are unique anyway.
		$resultXml = StringUtils::regexp_replace('/ xmlns="[^"]+"/', '', $resultXml);
