...
class IsbndbIsbnNlmCitationSchemaFilter extends IsbndbNlmCitationSchemaFilter {
	...
	function &process($isbn) {
		...
		$metadata['publisher-loc'] = StringUtils::trimPunctuation(StringUtils::regexp_replace('/^(.+):.*/', '\1', $metadata['place-publisher']));
		$metadata['publisher-name'] = StringUtils::trimPunctuation(StringUtils::regexp_replace('/.*:([^,]+),?.*/', '\1', $metadata['place-publisher']));
		unset($metadata['place-publisher']);

		// Reformat the publication date
		$metadata['date'] = StringUtils::regexp_replace('/^[^\d{4}]+(\d{4}).*/', '\1', $metadata['date']);

		// Clean non-numerics from ISBN
		$metadata['isbn'] = StringUtils::regexp_replace('/[^\dX]*/', '', $isbn);
