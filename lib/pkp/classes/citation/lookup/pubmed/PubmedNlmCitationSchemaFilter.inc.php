			...
			foreach($searchProperties as $nlmProperty => $pubmedProperty) {
				if (isset($statements[$nlmProperty])) {
					...
					if ($nlmProperty == 'person-group[@person-group-type="author"]') {
						...
						if (is_array($givenNames)) $searchTerms .= ' '.StringUtils::substr($givenNames[0], 0, 1);
					} else {
	...
	function &_lookup($pmid) {
		...
		foreach ($resultDOM->getElementsByTagName("Author") as $authorNode) {
			...
			if (!empty($givenNamesString)) {
				foreach(explode(' ', $givenNamesString) as $givenName) $authorDescription->addStatement('given-names', StringUtils::trimPunctuation($givenName));
			}
		...
		// Extract pagination
		if (StringUtils::regexp_match_get("/^[:p\.\s]*(?P<fpage>[Ee]?\d+)(-(?P<lpage>\d+))?/", $resultDOM->getElementsByTagName("MedlinePgn")->item(0)->textContent, $pages)) {
			...
			if (!empty($pages['lpage'])) {
				...
				if ($lPage < $fPage) {
					$lPage = (integer)(StringUtils::substr($pages['fpage'], 0, -StringUtils::strlen($pages['lpage'])).$pages['lpage']);
				}
		...
		if ($resultDOM->getElementsByTagName("PublicationType")->length > 0) {
			foreach($resultDOM->getElementsByTagName("PublicationType") as $publicationType) {
				...
				if (StringUtils::strpos(StringUtils::strtolower($publicationType->textContent), 'article') !== false) {
		...
		if(!is_null($resultDOM = $this->callWebService(PUBMED_WEBSERVICE_ELINK, $lookupParams))) {
			...
			foreach ($resultDOM->getElementsByTagName("ObjUrl") as $linkOut) {
				$attributes = '';
				foreach ($linkOut->getElementsByTagName("Attribute") as $attribute) $attributes .= StringUtils::strtolower($attribute->textContent).' / ';

				// Only add links to open access resources
				if (StringUtils::strpos($attributes, "subscription") === false && StringUtils::strpos($attributes, "membership") === false &&
						StringUtils::strpos($attributes, "fee") === false && $attributes != "") {
