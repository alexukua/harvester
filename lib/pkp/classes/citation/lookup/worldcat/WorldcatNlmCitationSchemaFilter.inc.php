...
class WorldcatNlmCitationSchemaFilter extends NlmCitationSchemaFilter {
	...
	function &process(&$citationDescription) {
		...
		foreach ($searchStrings as $searchString) {
			...
			StringUtils::regexp_match_all('/id="itemid_(\d+)"/', $result, $matches);
	...
	function _lookupWorldcat($oclcId) {
		...
		if (!empty($metadata['isbn'])) $metadata['isbn'] = StringUtils::regexp_replace('/[^\dX]*/', '', $metadata['isbn']);

		// Clean non-numerics from issued date (year)
		if (!empty($metadata['date'])) {
			$metadata['date'] = StringUtils::regexp_replace('/,.*/', ', ', $metadata['date']);
			$metadata['date'] = StringUtils::regexp_replace('/[^\d{4}]/', '', $metadata['date']);
		}
