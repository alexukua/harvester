...
class RegexRawCitationNlmCitationSchemaFilter extends NlmCitationSchemaFilter {
	...
	function process($citationString) {
		...
		if (StringUtils::regexp_match_get($urlPattern, $citationString, $matches)) {
			// Assume that the URL is a link to the resource.
			$metadata['uri'] = $matches[1];

			// Remove the URL from the citation string
			$citationString = StringUtils::regexp_replace($urlPattern, '', $citationString);
			...
			foreach ($pmIdExpressions as $pmIdExpression) {
				if (StringUtils::regexp_match_get($pmIdExpression, $matches[1], $pmIdMatches) ) {
		...
		if (StringUtils::regexp_match_get($pmidPattern, $citationString, $matches) ) {
			$metadata['pub-id[@pub-id-type="pmid"]'] = $matches[1];
			$citationString = StringUtils::regexp_replace($pmidPattern, '', $citationString);
		}

		// Parse out an embedded DOI and remove it from the citation string
		$doiPattern = '/doi:?\s*(\S+)/i';
		if (StringUtils::regexp_match_get($doiPattern, $citationString, $matches) ) {
			$metadata['pub-id[@pub-id-type="doi"]'] = $matches[1];
			$citationString = StringUtils::regexp_replace($doiPattern, '', $citationString);
		}

		// Parse out the access date if we have one and remove it from the citation string
		$accessDatePattern = '/accessed:?\s*([\s\w]+)/i';
		if (StringUtils::regexp_match_get($accessDatePattern, $citationString, $matches)) {
			$metadata['access-date'] = $matches[1];
			$citationString = StringUtils::regexp_replace($accessDatePattern, '', $citationString );
		}

		// Clean out square brackets
		$citationString = StringUtils::regexp_replace('/\[(\s*(pubmed|medline|full text)\s*)*]/i', '', $citationString);

		// Book citation
		$unparsedTail = '';
		if (StringUtils::regexp_match_get("/\s*(?P<authors>[^\.]+)\.\s*(?P<source>.*?)\s*(?P<publisherLoc>[^\.]*):\s*(?P<publisherName>[^:]*?);\s*(?P<date>\d\d\d\d.*?)(?P<tail>.*)/", $citationString, $matches)) {
		...
		// Journal citation
		} elseif (StringUtils::regexp_match_get("/\s*(?P<authors>[^\.]+)\.\s*(?P<titleSource>.*)\s*(?P<date>\d\d\d\d.*?);(?P<volumeAndIssue>[^:]+):(?P<tail>.*)/", $citationString, $matches)) {
			$metadata['[@publication-type]'] = NLM_PUBLICATION_TYPE_JOURNAL;
			$metadata['author'] = $matches['authors'];

			$titleSource = array();
			if (StringUtils::regexp_match_get("/(.*[\.!\?])(.*)/", trim($matches['titleSource'], " ."), $titleSource)) {
			...
			if (StringUtils::regexp_match_get("/([^\(]+)(\(([^\)]+)\))?/", $matches['volumeAndIssue'], $volumeAndIssue)) {
		...
		} elseif (StringUtils::regexp_match_get("/\s*(?P<citationSource>.*?)\s*URL:\s*(?P<tail>.*)/", $citationString, $matches)) {
			$unparsedTail = $matches['tail'];
		...
		if (!empty($unparsedTail) && StringUtils::regexp_match_get($pagesPattern, $unparsedTail, $matches)) {
			...
			$comment = StringUtils::trimPunctuation(StringUtils::regexp_replace($pagesPattern, '', $unparsedTail));
