...
class ParaciteRawCitationNlmCitationSchemaFilter extends NlmCitationSchemaFilter {
	...
	function &process($citationString) {
		...
		$citationString = StringUtils::utf8_to_ascii($citationString);
		...
		if ( Config::getVar('i18n', 'charset_normalization') == 'On' && !StringUtils::utf8_compliant($xmlResult) ) {
			$xmlResult = StringUtils::utf8_normalize($xmlResult);
		}
		...
		if (isset($metadata['authors'])) {
			$metadata['authors'] = StringUtils::trimPunctuation($metadata['authors']);
			$metadata['authors'] = StringUtils::iterativeExplode(array(':', ';'), $metadata['authors']);
		}
		...
		foreach(array('title', 'chapter', 'publication') as $titleProperty) {
			if (isset($metadata[$titleProperty])) $metadata[$titleProperty] = StringUtils::titleCase($metadata[$titleProperty]);
		}
		...
		foreach ($metadata as $paraciteElementName => $paraciteValue) {
			if (!empty($paraciteValue)) {
				// Trim punctuation
				if (is_string($paraciteValue)) $paraciteValue = StringUtils::trimPunctuation($paraciteValue);
		...
		if (isset($metadata['rest_text'])) {
			$nlmDescription->addStatement('comment', StringUtils::trimPunctuation($metadata['rest_text']));
		}
