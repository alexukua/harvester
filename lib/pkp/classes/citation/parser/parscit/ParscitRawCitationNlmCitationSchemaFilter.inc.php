...
class ParscitRawCitationNlmCitationSchemaFilter extends NlmCitationSchemaFilter {
	...
	function &process($citationString) {
		...
		// Detect errors.
		if (!StringUtils::regexp_match('/.*<algorithm[^>]+>.*<\/algorithm>.*/s', $result)) {
		...
		$xmlResult = StringUtils::regexp_replace('/.*<algorithm[^>]+>(.*)<\/algorithm>.*/s', '\1', html_entity_decode($result));
		$xmlResult = StringUtils::regexp_replace('/&/', '&amp;', $xmlResult);
