...
class Citation extends DataObject {
	...
	function _cleanCitationString($citationString) {
		...
		if (Config::getVar('i18n', 'charset_normalization') && !StringUtils::utf8_compliant($citationString)) {
			$citationString = StringUtils::utf8_normalize($citationString);
		}
		// 2) Strip slashes and whitespace
		$citationString = trim(stripslashes($citationString));

		// 3) Normalize whitespace
		$citationString = StringUtils::regexp_replace('/[\s]+/', ' ', $citationString);
