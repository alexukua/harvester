...
class PersonStringNlmNameSchemaFilter extends NlmPersonStringFilter {
	...
	function &_parsePersonsString($personsString, $title, $degrees) {
		// Check for 'et al'
		$personsStringBeforeEtal = StringUtils::strlen($personsString);
		$personsString = StringUtils::regexp_replace('/et ?al$/', '', $personsString);
		$etAl = ($personsStringBeforeEtal == StringUtils::strlen($personsString) ? false : true);

		// Remove punctuation
		$personsString = trim($personsString, ':;, ');

		// Cut the authors string into pieces
		$personStrings = StringUtils::iterativeExplode(array(':', ';'), $personsString);
		...
		if (count($personStrings) == 1) {
			if (StringUtils::regexp_match('/^((\w+\s+)+\w+\s*,)+\s*((\w+\s+)+\w+)$/i', $personStrings[0])) {
	...
	function &_parsePersonString($personString, $title, $degrees) {
		...
		$results = array();
		if ($title && StringUtils::regexp_match_get('/^('.$personRegex['title'].')/i', $personString, $results)) {
			$suffixString = trim($results[1], ',:; ');
			$personString = StringUtils::regexp_replace('/^('.$personRegex['title'].')/i', '', $personString);
		}

		if ($degrees && StringUtils::regexp_match_get('/('.$personRegex['degrees'].')$/i', $personString, $results)) {
			$degreesArray = explode(',', trim($results[1], ','));
			foreach($degreesArray as $key => $degree) {
				$degreesArray[$key] = StringUtils::trimPunctuation($degree);
			}
			$suffixString .= ' - '.implode('; ', $degreesArray);
			$personString = StringUtils::regexp_replace('/('.$personRegex['degrees'].')$/i', '', $personString);
		}
		...
		$personString = StringUtils::regexp_replace('/([A-Z])\.([A-Z][a-z])/', '\1. \2', $personString);
		...
		foreach ($personExpressions as $expressionId => $personExpression) {
			if ($nameFound = StringUtils::regexp_match_get($personExpression, $personString, $results)) {
				// Given names
				...
				if (!empty($results['initials'])) {
					...
					for ($initialNum = 0; $initialNum < StringUtils::strlen($results['initials']); $initialNum++) {
