...
class SQLParser {
	...
	function stripComments(&$sql) {
		$sql = trim(StringUtils::regexp_replace(sprintf('/^\s*%s(.*)$/m', $this->commentDelim), '', $sql));
	}
	...
	function &parseStatements(&$sql) {
		...
		for ($i=0, $count=count($statementsTmp); $i < $count; $i++) {
			...
			$numSingleQuotes += StringUtils::substr_count($statementsTmp[$i], "'");

			// Get number of escaped single quotes
			$numEscapedSingleQuotes += StringUtils::regexp_match_all("/(?<!\\\\)(\\\\\\\\)*\\\\'/", $statementsTmp[$i], $matches);
