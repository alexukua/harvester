...
define('CDN_JQUERY_VERSION', '1.4.1');
...
class PKPTemplateManager extends Smarty {
	...
	function smartyTruncate($string, $length = 80, $etc = '...', $break_words = false, $middle = false, $skip_tags = true) {
		...
		if (StringUtils::strlen($string) > $length) {
			$originalLength = StringUtils::strlen($string);
			if ($skip_tags) {
				if ($middle) {
					$tagsReverse = array();
			...
			$length -= min($length, StringUtils::strlen($etc));
			if (!$middle) {
				if(!$break_words) {
					$string = StringUtils::regexp_replace('/\s+?(\S+)?$/', '', StringUtils::substr($string, 0, $length+1));
				} else $string = StringUtils::substr($string, 0, $length+1);
				if ($skip_tags) $string = $this->_reinsertTags($string, $tags);
				return $this->_closeTags($string) . $etc;
			} else {
				$firstHalf = StringUtils::substr($string, 0, $length/2);
				$secondHalf = StringUtils::substr($string, -$length/2);
				...
				if($break_words) {
					...
					for($i=$length/2; $string[$i] != ' '; $i++) {
						$firstHalf = StringUtils::substr($string, 0, $i+1);
					}
					for($i=$length/2; StringUtils::substr($string, -$i, 1) != ' '; $i++) {
						$secondHalf = StringUtils::substr($string, -$i-1);
					}
	...
	function _removeTagsAux($string, $loc, &$tags, $length) {
		if(strlen($string) > 0 && $length > 0) {
			$length--;
			if(StringUtils::substr($string, 0, 1) == '<') {
				$closeBrack = StringUtils::strpos($string, '>')+1;
				if($closeBrack) {
					$tags[] = array(StringUtils::substr($string, 0, $closeBrack), $loc);
					return $this->_removeTagsAux(StringUtils::substr($string, $closeBrack), $loc+$closeBrack, $tags, $length);
				}
			}
			return StringUtils::substr($string, 0, 1) . $this->_removeTagsAux(StringUtils::substr($string, 1), $loc+1, $tags, $length);
		}
	}
	...
	function _removeTagsAuxReverse($string, $loc, &$tags, $length) {
		$backLoc = StringUtils::strlen($string)-1;
		if($backLoc >= 0 && $length > 0) {
			$length--;
			if(StringUtils::substr($string, $backLoc, 1) == '>') {
				$tag = '>';
				$openBrack = 1;
				while (StringUtils::substr($string, $backLoc-$openBrack, 1) != '<') {
					$tag = StringUtils::substr($string, $backLoc-$openBrack, 1) . $tag;
					$openBrack++;
				}
				$tag = '<' . $tag;
				$openBrack++;
				...
				$tags[] = array($tag, $loc);
				return $this->_removeTagsAuxReverse(StringUtils::substr($string, 0, -$openBrack), $loc+$openBrack, $tags, $length);
			}
			return $this->_removeTagsAuxReverse(StringUtils::substr($string, 0, -1), $loc+1, $tags, $length) . StringUtils::substr($string, $backLoc, 1);
		}
	}
	...
	function _reinsertTags($string, &$tags, $reverse = false) {
		...
		for($i = 0; $i < count($tags); $i++) {
			...
			if ($tags[$i][1] < StringUtils::strlen($string)) {
				if ($reverse) {
					if ($tags[$i][1] == 0) { // Cannot use -0 as the start index (its same as +0)
						$string = StringUtils::substr_replace($string, $tags[$i][0], $length, 0);
					} else {
						$string = StringUtils::substr_replace($string, $tags[$i][0], -$tags[$i][1], 0);
					}
				} else {
					$string = StringUtils::substr_replace($string, $tags[$i][0], $tags[$i][1], 0);
				}
	...
	function _closeTags($string, $open = false){
		...
		StringUtils::regexp_match_all("#<([a-z]+)( .*)?(?!/)>#iU", $string, $result);
		$openedtags = $result[1];
	...
	function smartyLoadUrlInDiv($params, &$smarty) {
		...
		if (isset($params['loadMessageId'])) {
			...
			$translatedLoadMessage = __('common.loading');
		}
