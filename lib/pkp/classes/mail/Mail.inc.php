...
class Mail extends DataObject {
	...
	function addAttachment($filePath, $fileName = '', $contentType = '', $contentDisposition = 'attachment') {
		...
		if (empty($contentType)) {
			$contentType = StringUtils::mime_content_type($filePath);
	...
	function send() {
		...
		$subject = StringUtils::encode_mime_header($this->getSubject());
		$body = $this->getBody();

		// FIXME Some *nix mailers won't work with CRLFs
		if (Core::isWindows()) {
			// Convert LFs to CRLFs for Windows
			$body = StringUtils::regexp_replace("/([^\r]|^)\n/", "\$1\r\n", $body);
		} else {
			// Convert CRLFs to LFs for *nix
			$body = StringUtils::regexp_replace("/\r\n/", "\n", $body);
		}
		...
		if (Config::getVar('email', 'smtp')) {
			...
			$sent = StringUtils::mail($recipients, $subject, $mailBody, $headers, $additionalParameters);
		}
	...
	function encodeDisplayName($displayName) {
		if (StringUtils::regexp_match('!^[-A-Za-z0-9\!#\$%&\'\*\+\/=\?\^_\`\{\|\}~]+$!', $displayName)) return $displayName;
		return ('"' . str_replace(
			array('"', '\\'),
			'',
			$displayName
