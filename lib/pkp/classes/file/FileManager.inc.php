...
class FileManager {
	...
	function getUploadedFileType($fileName) {
		if (isset($_FILES[$fileName])) {
			$type = StringUtils::mime_content_type($_FILES[$fileName]['tmp_name']);
	...
	function downloadFile($filePath, $type = null, $inline = false) {
		...
		if (is_readable($filePath)) {
			if ($type == null) {
				$type = StringUtils::mime_content_type($filePath);
	...
	function truncateFileName($fileName, $length = 127) {
		if (StringUtils::strlen($fileName) <= $length) return $fileName;
		$ext = FileManager::getExtension($fileName);
		$truncated = StringUtils::substr($fileName, 0, $length - 1 - StringUtils::strlen($ext)) . '.' . $ext;
		return StringUtils::substr($truncated, 0, $length);
	}
