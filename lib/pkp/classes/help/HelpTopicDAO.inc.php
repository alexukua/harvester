...
class HelpTopicDAO extends XMLDAO {
	...
	function &getTopicsByKeyword($keyword) {
		$keyword = StringUtils::strtolower($keyword);
	...
	function scanTopic(&$mappingFile, &$matchingTopics,$keyword,$dir,$file) {
		if (preg_match('/^\d{6,6}\.xml$/', $file)) {
			...
			if ($topic) {
				$numMatches = StringUtils::substr_count(StringUtils::strtolower($topic->getTitle()), $keyword);

				foreach ($topic->getSections() as $section) {
					$numMatches += StringUtils::substr_count(StringUtils::strtolower($section->getTitle()), $keyword);
					$numMatches += StringUtils::substr_count(StringUtils::strtolower($section->getContent()), $keyword);
				}
