...
class XMLParser {
	...
	function __construct() {
		$this->errors = array();
        $this->log = new Logger('XMLParser');
        $this->log->pushHandler(new StreamHandler(BASE_SYS_DIR.'/log/xmlparcer.log', Logger::DEBUG));
	...
	function parseText($text) {
		...
		if ( Config::getVar('i18n', 'charset_normalization') == 'On' && !StringUtils::utf8_compliant($text) ) {

			$text = StringUtils::utf8_normalize($text);

			// strip any invalid UTF-8 sequences
			$text = StringUtils::utf8_bad_strip($text);

			// convert named entities to numeric entities
			$text = strtr($text, StringUtils::getHTMLEntities());
		}

		// strip any invalid ASCII control characters
		$text = StringUtils::utf8_strip_ascii_ctrl($text);

		if (!xml_parse($parser, $text, true)) {
			$this->addError(xml_error_string(xml_get_error_code($parser)).__LINE__);
		}
	...
	public function parse($file, $dataCallback = null) {
		...
		if (!$wrapper->open()){
			$this->addError('Error open'.$file.__LINE__.__FILE__);
		}
		if (!$wrapper) {
			$result = false;
			return $result;
		...
		while (!$wrapper->eof() && ($data = $wrapper->read()) !== false) {
			...
			// if the string contains non-UTF8 characters, convert it to UTF-8 for parsing
			if ( Config::getVar('i18n', 'charset_normalization') == 'On' && !StringUtils::utf8_compliant($data) ) {

				$utf8_last = StringUtils::substr($data, StringUtils::strlen($data) - 1);

				// if the string ends in a "bad" UTF-8 character, maybe it's truncated
				while (!$wrapper->eof() && StringUtils::utf8_bad_find($utf8_last) === 0) {
					// read another chunk of data
					$data .= $wrapper->read();
					$utf8_last = StringUtils::substr($data, StringUtils::strlen($data) - 1);
				}

				$data = StringUtils::utf8_normalize($data);

				// strip any invalid UTF-8 sequences
				$data = StringUtils::utf8_bad_strip($data);

				// convert named entities to numeric entities
				$data = strtr($data, StringUtils::getHTMLEntities());
			}

			// strip any invalid ASCII control characters
			$data = StringUtils::utf8_strip_ascii_ctrl($data);

			//file_put_contents('/var/www/html/data.txt', $data, FILE_APPEND);

			if ($dataCallback) call_user_func($dataCallback, 'parse', $wrapper, $data);
			if (!xml_parse($parser, $data, $wrapper->eof())) {
				$this->addError(xml_error_string(xml_get_error_code($parser)).__LINE__.__FILE__);
			}
		}
