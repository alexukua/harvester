...
/**
 * @file classes/core/StringUtils.inc.php
 *
...
class StringUtils {
	...
	function init() {
		...
		if ($clientCharset == 'utf-8' && StringUtils::hasPCREUTF8()) {
			define('PCRE_UTF8', 'u');
		} else {
			define('PCRE_UTF8', '');
		}
		...
		if (function_exists('mb_substr_replace') === false) {
			function mb_substr_replace($string, $replacement, $start, $length = null) {
				if (extension_loaded('mbstring') === true) {
					$string_length = StringUtils::strlen($string);
					...
					return StringUtils::substr($string, 0, $start) . $replacement . StringUtils::substr($string, $start + $length, $string_length - $start - $length);
				}
			}
	...
	function regexp_grep($pattern, $input) {
		if (PCRE_UTF8 && !StringUtils::utf8_compliant($input)) $input = StringUtils::utf8_bad_strip($input);
	...
	function regexp_match($pattern, $subject) {
		if (PCRE_UTF8 && !StringUtils::utf8_compliant($subject)) $subject = StringUtils::utf8_bad_strip($subject);
	...
	function html2text($html) {
		$html = StringUtils::regexp_replace('/<[\/]?p>/', "\n", $html);
		$html = StringUtils::regexp_replace('/<li>/', '&bull; ', $html);
		$html = StringUtils::regexp_replace('/<\/li>/', "\n", $html);
		$html = StringUtils::regexp_replace('/<br[ ]?[\/]?>/', "\n", $html);
		$html = StringUtils::html2utf(strip_tags($html));
		return $html;
	}
	...
	function html2utf($str) {
		...
		$str = strtr($str, StringUtils::getHTMLEntities());

		// use PCRE-aware replace function to replace numeric entities
		$str = StringUtils::regexp_replace('~&#x([0-9a-f]+);~ei', 'StringUtils::code2utf(hexdec("\\1"))', $str);
		$str = StringUtils::regexp_replace('~&#([0-9]+);~e', 'StringUtils::code2utf(\\1)', $str);
	...
	function uncamelize($string) {
		...
		$words = array();
		StringUtils::regexp_match_all('/[A-Z][a-z0-9]*/', $string, $words);
	...
	function diff($originalString, $editedString) {
		...
		foreach(array('originalStringCharacters' => $originalString, 'editedStringCharacters' => $editedString) as $characterArrayName => $string) {
			...
			StringUtils::regexp_match_all('/./', $string, ${$characterArrayName});
			...
			if (isset($characterIndex[$comparedCharacter])) {
				...
				foreach($characterIndex[$comparedCharacter] as $editedPosition) {
					...
					if (isset($substringIndex[$previousPosition])) {
						...
						$newSubstringLength = StringUtils::strlen($newSubstring);
		...
		$precedingSubstringOriginal = StringUtils::substr($originalString, 0, $largestSubstringEndOriginal-$largestSubstringLength+1);
		$precedingSubstringEdited = StringUtils::substr($editedString, 0, $largestSubstringEndEdited-$largestSubstringLength+1);
		$diffResult = array_merge(StringUtils::diff($precedingSubstringOriginal, $precedingSubstringEdited), $diffResult);

		// Append the diff of the substrings after thr common substring
		// to the result diff (by recursion).
		$succeedingSubstringOriginal = StringUtils::substr($originalString, $largestSubstringEndOriginal+1);
		$succeedingSubstringEdited = StringUtils::substr($editedString, $largestSubstringEndEdited+1);
		$diffResult = array_merge($diffResult, StringUtils::diff($succeedingSubstringOriginal, $succeedingSubstringEdited));
