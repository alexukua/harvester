...
class Core {
	...
	function cleanVar($var) {
		...
		if ( Config::getVar('i18n', 'charset_normalization') == 'On' && strtolower(Config::getVar('i18n', 'client_charset')) == 'utf-8' && !StringUtils::utf8_is_valid($var) ) {

			$var = StringUtils::utf8_normalize($var);

			// convert HTML entities into valid UTF-8 characters (do not transcode)
			if (checkPhpVersion('5.0.0')) {
				$var = html_entity_decode($var, ENT_COMPAT, 'UTF-8');
			} else {
				$var = StringUtils::html2utf($var);
			}

			// strip any invalid UTF-8 sequences
			$var = StringUtils::utf8_bad_strip($var);
		...
		$var = StringUtils::utf8_strip_ascii_ctrl($var);
	...
	function cleanFileVar($var) {
		return StringUtils::regexp_replace('/[^\w\-]/', '', $var);
	}
