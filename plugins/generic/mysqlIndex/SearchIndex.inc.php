...
class SearchIndex {
	...
	function &filterKeywords($text, $allowWildcards = false) {
		...
		$cleanText = StringUtils::regexp_replace('/[!"\#\$%\'\(\)\.\?@\[\]\^`\{\}~]/', '', $text);
		$cleanText = StringUtils::regexp_replace('/[\+,:;&\/<=>\|\\\]/', ' ', $cleanText);
		$cleanText = StringUtils::regexp_replace('/[\*]/', $allowWildcards ? '%' : ' ', $cleanText);
		$cleanText = StringUtils::strtolower($cleanText);

		// Split into words
		$words = StringUtils::regexp_split('/\s+/', $cleanText);
		...
		$keywords = array();
		foreach ($words as $k) {
			if (!isset($stopwords[$k]) && StringUtils::strlen($k) >= $minLength && !is_numeric($k)) {
				$keywords[] = StringUtils::substr($k, 0, $maxLength);
			}
		}
		return $keywords;
	}
