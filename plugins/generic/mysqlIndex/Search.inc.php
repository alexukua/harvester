...
class Search {
	...
	function _parseQuery($signTokens, $tokens, &$pos, $total) {
		...
		$notOperator = StringUtils::strtolower(__('search.operator.not'));
		$andOperator = StringUtils::strtolower(__('search.operator.and'));
		$orOperator = StringUtils::strtolower(__('search.operator.or'));
		while ($pos < $total) {
			if (!empty($signTokens[$pos])) $sign = $signTokens[$pos];
			else if (empty($sign)) $sign = '+';
			$token = StringUtils::strtolower($tokens[$pos++]);
			switch ($token) {
				case $notOperator:
					$sign = '-';
					break;
				...
				default:
					$postBool = '';
					if ($pos < $total) {
						$peek = StringUtils::strtolower($tokens[$pos]);
						if ($peek == $orOperator) {
							$postBool = 'or';
							$pos++;
